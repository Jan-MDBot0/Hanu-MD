...
function hanuLog(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [HANU-MD] ${new Date().toISOString()}: ${message}`);
}
...
async function HanuPair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `hanu_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            hanuLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                const msg = await hanuStore.loadMessage(key.remoteJid, key.id);
                return msg && msg.message ? msg.message : { conversation: 'HANU-MD' };
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363426828465969@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        ...
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© Hanu-MD' } }
                    }},
            ...
            } catch (e) { hanuLog(`Message handler error: ${e.message}`, 'error'); }
        });

    } catch (err) {
        arslanLog(`HANU-MD Pair error: ${err.message}`, 'error');
...
router.get('/code', async (req, res) => { if (!req.query.number) return res.json({ error: 'Number required' }); await hanuPair(req.query.number, res); });
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 HANU-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        ...
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { hanuLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        HanuLog('Auto-reconnect completed', 'success');
    } catch (e) { hanuLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}
